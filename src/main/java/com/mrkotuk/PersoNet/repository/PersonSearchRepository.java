...
public interface PersonSearchRepository extends JpaRepository<Person, Integer> {
    ...
    Set<String> findTagsByEmail(@Param("email") String email);

    @Query("SELECT p.personType FROM Person p WHERE p.email = :email")
    Set<PersonType> findTypesByEmail(@Param("email") String email);

    @Query("SELECT p.personStatus FROM Person p WHERE p.email = :email")
    Set<PersonStatus> findStatusesByEmail(@Param("email") String email);
}
